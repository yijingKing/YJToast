...
//  YJToast
//
//  Created by 1091676312@qq.com on 10/26/2022.
...
class ViewController: UIViewController {
    ...
    let contentList: [ToastContentType] = [
        ...
        ToastImageContent(image: .gif(name: "hud_loading"))
    ]
    ...
    @objc func showToast() {
        ...
        if let _ = currentContent as? ToastMessageContent {
            toast.show(content: ToastMessageContent(message: "文本消息"), position: selectedPosition, retainTime: selectedTime)
        }
        if let _ = currentContent as? ToastImageContent {
            toast.show(content: ToastImageContent(image: UIImage.gif(name: "hud_loading")), position: selectedPosition, retainTime: selectedTime)
        }
    }
